...
gb%>%filter(species!="Unknown")%>%distinct(class)


#numberof host species

gb%>%distinct(species)%>%filter(species!="Unknown")%>%nrow() 
...
gb%>%filter(species!="Unknown")%>%distinct(order)%>%nrow()
...
gb%>%filter(species!="Unknown")%>%distinct(family)%>%nrow()
...
# family of bats sourcing sarbecovirus  
  ...
  aves<-read.csv("/Users/DMontecino/Documents/CoV_Wildlife/Data/number of species by class and order Aves IUCN.csv")  
  mammals<-read.csv("/Users/DMontecino/Documents/CoV_Wildlife/Data/number of species by class and order Mammals IUCN.csv")  
